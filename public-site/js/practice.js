...
function showSummary() {
    ...
    if (window.MotivationalMessages) {
        const msg = window.MotivationalMessages.getMessage(correct, total);
        const motDiv = document.getElementById('motivationalMessage');
        if (motDiv) {
            motDiv.innerHTML = `
                <div style="font-size: 2.5rem;">${msg.emoji}</div>
                <h3 style="margin: 0.5rem 0; color: #1a1a2e;">${msg.quote}</h3>
                <p style="color: #4a5568;">${msg.message}</p>
            `;
        }
    }
...
function sharePracticeResults() {
    ...
    const total = practiceState.questions.length;
    const acc = Math.round((correct / total) * 100);
    const text = `I scored ${correct}/${total} (${acc}%) in JAMB practice! 🎯\n\nPractice with JAMB Simulator 2026!`;
